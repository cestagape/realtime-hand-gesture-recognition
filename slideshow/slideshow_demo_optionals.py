...
from helpers import output_to_gesture_optionals
...
async def emitter(_request, ws):
    ...
    # Modell laden
    scaler = mf.StandardScaler.load(filename="scaler_optionals.npz")
    my_model = mf.Model()
    my_model.add(mf.Dense(250, 256, activation="sigmoid"))  
    my_model.add(mf.Dense(256, 128, activation="sigmoid"))
    my_model.add(mf.Dense(128, 12))                                       
    my_model.load(filename="model_optionals.npz")
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                pose_data = [
                    landmarks[13].x, landmarks[13].y, landmarks[13].z, landmarks[13].visibility,  # Left elbow
                    landmarks[15].x, landmarks[15].y, landmarks[15].z, landmarks[15].visibility,  # Left wrist
                    landmarks[14].x, landmarks[14].y, landmarks[14].z, landmarks[14].visibility,  # Right elbow
                    landmarks[16].x, landmarks[16].y, landmarks[16].z, landmarks[16].visibility,  # Right wrist
                    landmarks[19].x, landmarks[19].y, landmarks[19].z, landmarks[19].visibility,  # Left index
                    landmarks[20].x, landmarks[20].y, landmarks[20].z, landmarks[20].visibility   # Right index
                ]
            ...
            if len(pose_data_queue) == frame_window_size and looking > 0.93:
                ...
                gesture = output_to_gesture_optionals(my_model.predict(X_scaled))
